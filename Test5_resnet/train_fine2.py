        ...
        for step, data in enumerate(train_bar):
            ...
            for i in range(len(labels)):
                if (judge(labels[i], category[0])):
                    ...
                    tag2img.append(i)
                    tag2num += 1
            images_cls2 = torch.zeros(tag1num, 3, 224, 224)
            label_cls2 = torch.zeros(tag1num).long()

            for i in range(tag1num):
                images_cls2[i] = images[tag1img[i]]
                label_cls2[i] = tag1[i]

            optimizer.zero_grad()
            logits = net(images_cls2.to(device))
            loss = loss_function(logits, label_cls2.to(device))
            loss.backward()
            optimizer.step()
